...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-xl border border-transparent bg-card text-card-foreground shadow transition-all duration-300 ease-in-out transform hover:scale-105 hover:bg-opacity-90 card",
        className
      )}
      {...props}
    />
