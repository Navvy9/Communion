...
const CustomCarousel: React.FC = () => {
  ...
  return (
    <div className="relative flex flex-col justify-center items-center max-h-screen rounded-full bg-gradient-to-b from-indigo-500 via-purple-500 to-pink-500 py-10 ">
      <h1 className="text-3xl text-white font-semibold mb-8">Divine Destinations</h1>

      {/* Previous Button */}
      <motion.button
        onClick={handlePrev}
        whileHover={{ scale: 1.1 }}
        className="absolute left-5 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-full shadow-lg"
      >
        &lt;
      </motion.button>
      ...
      <div className="flex space-x-8">
        {cardData.slice(currentIndex, currentIndex + 3).map((card) => (
          ...
          >
            <div className="w-64 h-64 flex justify-center items-center border-2 border-white rounded-lg overflow-hidden shadow-2xl bg-white">
      ...
      <motion.button
        onClick={handleNext}
        whileHover={{ scale: 1.1 }}
        className="absolute right-5 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-full shadow-lg"
      >
        &gt;
      </motion.button>
      ...
      <AnimatePresence>
        {isModalOpen && selectedCard && (
          ...
          >
            <motion.div
              className="bg-white p-6 rounded-lg w-3/4 relative shadow-2xl"
              initial={{ scale: 0.8 }}
              animate={{ scale: 1 }}
              exit={{ scale: 0.8 }}
            >
              ...
              <div className="flex flex-col md:flex-row space-y-8 md:space-x-8 justify-center items-center">
                {/* Image of the clicked card */}
                <motion.div
                  className="w-80 h-80 flex justify-center items-center border border-gray-300 rounded-lg overflow-hidden shadow-lg"
                ...
                <motion.div
                  className="w-80 h-80 flex flex-col justify-center items-center p-4 bg-gradient-to-b from-gray-100 to-gray-200 border rounded-lg shadow-lg text-center"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                >
                  <p className="text-lg font-medium text-gray-800">{selectedCard.description}</p>
                </motion.div>
              </div>
            </motion.div>
          </motion.div>
